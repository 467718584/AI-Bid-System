...
public interface MaterialMapper {
    ...
    @Insert("INSERT INTO bid_material (name, material_type, project_id, file_path, file_size, file_type, upload_user_id, status, remark, deleted, create_by, create_time) " +
            "VALUES (#{name}, #{materialType}, #{projectId}, #{filePath}, #{fileSize}, #{fileType}, #{uploadUserId}, #{status}, #{remark}, 0, #{createBy}, #{createTime})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(BidMaterial material);

    @Update("UPDATE bid_material SET name=#{name}, material_type=#{materialType}, project_id=#{projectId}, file_path=#{filePath}, " +
