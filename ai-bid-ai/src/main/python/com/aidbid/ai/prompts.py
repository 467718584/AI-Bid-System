...
# 图文并茂要求
当内容需要图表说明时，请使用以下Markdown图片标记语法：
![图表标题](chart:bar|line|pie|<json数据>)

示例：
![施工进度计划表](chart:bar|项目进度|阶段1,阶段2,阶段3|进度30%,进度65%,进度90%|已完成,进行中,未完成)

对于数据表格，使用以下标记：
![表格标题](table:<json数据>)

示例：
![设备配置表](table:[{{"headers":["设备名称","数量","规格"],"rows":[["挖掘机","3台","卡特320"],["推土机","2台","D8型"]]}}])

# 要求
1. 内容要紧扣招标要求和评分标准
2. 使用专业、规范的技术语言
3. 适当插入图表（当需要展示数据时优先使用图表）
4. 总页数控制在 {page_count} 页以内
5. 每500字左右考虑插入一个相关图表
